...
const STATUS = {
    paid: { label: 'Đã đóng', color: '#10B981', bg: 'rgba(16,185,129,0.1)' },
    partial: { label: 'Đóng thiếu', color: '#F59E0B', bg: 'rgba(245,158,11,0.1)' },
    unpaid: { label: 'Chưa đóng', color: '#EF4444', bg: 'rgba(239,68,68,0.1)' },
}
    ...
    return (
        <div style={{ paddingBottom: 40 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px 12px' }}>
                ...
                <span style={{ fontWeight: 700, fontSize: '16px', color: '#1A1A2E', textAlign: 'center' }}>
...
export default function RoundDetail({ store, navigate, params }) {
    ...
    if (!round) return <div style={{ padding: '40px', textAlign: 'center', color: '#9CA3AF' }}>Không tìm thấy đợt thu.</div>
    ...
    const markAllPaid = () => {
        if (!window.confirm('Đánh dấu tất cả đã đóng?')) return
        roundPayments.filter(p => p.status !== 'paid').forEach(p =>
            recordPayment(roundId, p.studentId, { paidAmount: round.amount, paidDate: today(), method: 'cash', status: 'paid', note: '' })
        )
        Toast.show({ icon: 'success', content: 'Đã đánh dấu tất cả!' })
    }
        ...
        <div style={{ paddingBottom: '8px' }}>
            ...
            <div style={{ background: '#fff', padding: '16px 20px 20px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
                    <div>
                        <div style={{ fontWeight: 800, fontSize: '22px', letterSpacing: '-0.5px', color: '#1A1A2E' }}>{round.name}</div>
                        <div style={{ fontSize: '13px', color: '#9CA3AF', marginTop: '3px' }}>
                            {cls?.name} · Hạn {fmtDate(round.dueDate)} · {fmt(round.amount)}/HS
                        </div>
                    </div>
                    <span style={{ fontSize: '11px', fontWeight: 700, padding: '4px 10px', borderRadius: '100px', background: `${accent}10`, color: accent, flexShrink: 0, marginTop: '2px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '7px', marginBottom: '14px' }}>
                    {[
                        { l: 'Tổng', v: stats.total, c: '#1A1A2E' },
                        { l: 'Đã đóng', v: stats.paid, c: '#10B981' },
                        { l: 'Thiếu', v: stats.partial, c: '#F59E0B' },
                        { l: 'Chưa', v: stats.unpaid, c: '#EF4444' },
                    ].map(i => (
                        <div key={i.l} style={{ background: '#F9FAFB', borderRadius: '10px', padding: '8px', textAlign: 'center', border: '1px solid rgba(0,0,0,0.05)' }}>
                            <div style={{ fontSize: '11px', color: '#9CA3AF', fontWeight: 600, marginBottom: '3px' }}>{i.l}</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '7px', marginBottom: '14px' }}>
                    {[
                        ...
                        { l: 'Cần thu', v: fmt(stats.totalExpected), c: '#1A1A2E' },
                        { l: 'Còn thiếu', v: fmt(stats.totalExpected - stats.totalCollected), c: '#EF4444' },
                    ].map(i => (
                        <div key={i.l} style={{ background: '#F9FAFB', borderRadius: '10px', padding: '10px', textAlign: 'center', border: '1px solid rgba(0,0,0,0.05)' }}>
                            <div style={{ fontSize: '11px', color: '#9CA3AF', fontWeight: 600, marginBottom: '3px' }}>{i.l}</div>
                ...
                {/* Progress */}
                <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#9CA3AF', fontWeight: 600, marginBottom: '6px' }}>
                        <span>Tiến độ</span>
                        <span style={{ color: '#1A1A2E' }}>{pct}%</span>
                    </div>
                    <ProgressBar percent={pct} style={{ '--fill-color': accent, '--track-color': 'rgba(0,0,0,0.07)', '--track-width': '8px' }} />
                </div>
            </div>

            {/* Filter pills */}
            <div style={{ padding: '12px 16px', display: 'flex', gap: '7px', flexWrap: 'wrap', alignItems: 'center', background: '#fff', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                {FILTER_PILLS.map(({ v, l }) => (
                    <button
                        ...
                        style={{
                            ...
                            border: filterStatus === v ? 'none' : '1px solid rgba(0,0,0,0.1)',
                            background: filterStatus === v ? '#3B82F6' : '#fff',
            ...
            {/* Student list */}
            <div style={{ background: '#fff', margin: '12px 16px', borderRadius: '14px', overflow: 'hidden', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                        ...
                        <div key={p.id} style={{
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
                                    <span style={{ fontWeight: 700, fontSize: '14px', color: '#1A1A2E' }}>{s.name}</span>
                                ...
                                {p.paidDate && (
                                    <div style={{ fontSize: '11px', color: '#D1D5DB', marginTop: '1px' }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    border: p.status === 'paid' ? '1px solid rgba(0,0,0,0.1)' : 'none',
                                    background: p.status === 'paid' ? '#F9FAFB' : '#3B82F6',
                                    color: p.status === 'paid' ? '#6B7280' : '#fff',
                ...
                {rows.length === 0 && <div style={{ padding: '28px', textAlign: 'center', color: '#9CA3AF', fontSize: '14px' }}>Không có học sinh</div>}

                {/* Footer total */}
                {rows.length > 0 && (
                    <div style={{ padding: '12px 16px', background: '#F9FAFB', borderTop: '1px solid rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '12px', fontWeight: 600, color: '#9CA3AF' }}>{rows.length} học sinh</span>
                        <div style={{ fontSize: '13px' }}>
                            <span style={{ color: '#10B981', fontWeight: 800 }}>{fmt(rows.reduce((s, r) => s + (r.payment.paidAmount || 0), 0))}</span>
                            <span style={{ color: '#D1D5DB' }}> / {fmt(rows.reduce((s, r) => s + (r.payment.requiredAmount || round.amount), 0))}</span>
                        </div>
                    </div>
                )}
            </div>
